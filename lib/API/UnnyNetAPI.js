...
export default class UnnyNetAPI {
    ...
    authWithVK(userId, token, callback) {
        if (!userId)
            return console.error("[VK]: no viewer_id provided");
        if (!token)
            return console.error("[VK]: no auth_key provided");
    ...
    authWithOK(env, accessToken, sessionSecretKey, callback) {
        if (!accessToken)
            return console.error("[OK]: no access_token provided");
        if (!sessionSecretKey)
            return console.error("[OK]: no session_secret_key provided");

        UnnySettings.clearTokens();
        const request = new UnnyNetAPIRequest(API_URL + '/v1/auth/ok', {
            env,
    ...
    authWithFB(env, token, callback) {
        if (!env)
            return console.error("[FB]: no environment provided");
        if (!token)
            return console.error("[FB]: no auth_key provided");

        UnnySettings.clearTokens();
        const request = new UnnyNetAPIRequest(API_URL + '/v1/auth/fb', {
            env,
            token,
            game_id:    this.config.game_id,
            public_key: this.config.public_key
    ...
    purchaseProduct(id, callback) {
        ...
        itemParams1[opensocial.BillingItem.Field.PRICE] = 1000;
