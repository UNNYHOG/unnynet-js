...
export default class UnnyNetAPI {
    ...
    _onAuthCallback(err, data, callback) {
        let response;
        if (err) {
            response = UnnynetCommand.getErrorResponse(Errors.Unknown, err.status + ':' + err.statusText);
        } else {
            response = UnnynetCommand.getSuccessResponse(data);
            this._onAuth(data);
        }
        if (callback)
            callback(response);
    ...
    authWithNutakuGadget(callback) {
        if (typeof gadgets !== 'undefined') {
            ...
            console.warn("AUTH with login");
            this.authWithLogin('test_user1', 'test_password', callback);
        }
    }
