...
export default class Payments {
     ...
     */
    static purchaseProduct(item_id, doneCallback) {
        if (!doneCallback) {
        ...
        Payments.getProducts(productsResponse => {
            console.info("getProducts", productsResponse);
            if (productsResponse.success) {
                const productInfo = Payments._getProductById(item_id);
                if (!productInfo) {
                    doneCallback(UnnynetCommand.getErrorResponse(Errors.NoSuchProduct, "No such product with Id: " + item_id));
                } else {
    ...
    static restorePurchases(doneCallback) {
        ...
        Payments.getPurchases(UnnyConstants.PurchaseStatusFilter.COMPLETED, (completedResponse) => {
            console.info(">>>completedResponse ", completedResponse);
        ...
        Payments.getPurchases(UnnyConstants.PurchaseStatusFilter.PENDING, (pendingResponse) => {
            console.info(">>>pendingResponse ", pendingResponse);
                ...
                for (let pendingItem of pendingPurchases) {
                    ...
                    if (un_order_id) {
                        Payments.completePurchase(un_order_id, (completeResponse) => {
                            console.info(">>>completePurchase ", completeResponse);
                            if (completeResponse.success) {
    ...
    static _getProductById(item_id) {
        return Payments.cachedProducts.formattedProducts[item_id];
    }

    static _createFormattedProducts(products) {
        const formattedProducts = {};
        for (let product in products)
            formattedProducts[product.item_id] = product;
