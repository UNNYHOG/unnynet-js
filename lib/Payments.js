...
export default class Payments {
     ...
     */
    static purchaseProduct(product_id, doneCallback) {
        if (!doneCallback) {
        ...
        Payments.getProducts(productsResponse => {
            if (productsResponse.success) {
                const productInfo = Payments._getProductById(product_id);
                if (!productInfo) {
                    doneCallback(UnnynetCommand.getErrorResponse(Errors.NoSuchProduct, "No such product with Id: " + product_id));
                } else {
    ...
    static _getProductById(product_id) {
        return Payments.cachedProducts.formattedProducts[product_id];
    }

    static _createFormattedProducts(products) {
        const formattedProducts = {};
        for (let product of products) {
            product.product_id = product.base_id;
            product.platform_product_id = product.item_id;
            formattedProducts[product.product_id] = product;
        }
