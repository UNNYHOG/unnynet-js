...
//region API
     ...
     * @param {function} doneCallback
     */
    static reportLeaderboards(leaderboardId, newScore, doneCallback) {
        const code = UnnyCommands.GetCommand(UnnyCommands.Command.ReportLeaderboardScores).format(leaderboardId, newScore);
        UnnyNetBase.evaluateCodeInJavaScript(new UnnyCommandInfo(UnnyCommands.Command.ReportLeaderboardScores, code, false, doneCallback));
    }
    ...
    static reportAchievements(achId, progress, doneCallback) {
        const code = UnnyCommands.GetCommand(UnnyCommands.Command.ReportAchievementProgress).format(achId, progress);
        UnnyNetBase.evaluateCodeInJavaScript(new UnnyCommandInfo(UnnyCommands.Command.ReportAchievementProgress, code, false, doneCallback));
    }
